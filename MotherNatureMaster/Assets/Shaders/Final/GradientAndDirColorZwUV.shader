Shader "Custom/GradientAndDirColorZwUV" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
        	...
        	// 	Gradient color
         	half3 gradient = lerp(_ColorLow, _ColorHigh,  smoothstep( _zPosLow, _zPosHigh, IN.worldPos.z )).rgb;
         	gradient = lerp(WHITE3, gradient, _GradientStrength);		
         	
         	//	Directional color
         	half3 finalColor = _ColorZ.rgb * max(0,dot(o.Normal, RIGHT))* _ColorZ.a;		// Add ColorZ if the normal is facing positive X-ish (right)
