...
public class Level9GameController : MonoBehaviour {
    public Transform kid, icon1, icon2, crate1, crate2, door;
    ...
    private Vector3 buttonVec1 = new Vector3(-4f, 12f, -5f);
    private Vector3 buttonVec2 = new Vector3(-5f, 1f, -3f);
    ...
    void Update() {
        ...
        if (complete)
        {
            door.position = Vector3.MoveTowards(door.position, new Vector3(-1f, 5f, -6f), 2f * Time.deltaTime);
        }
        else
        {
            door.position = Vector3.MoveTowards(door.position, new Vector3(-1f, 9f, -6f), 2f * Time.deltaTime);
        }
    }
